...
- The selected visual target is ideation option 3, the media-draft chat layout in `reference-option-3.png`.
- The default first screen is a standalone chat with a persistent message field, a `전송` action, and a short `분석` entry action.
- Chat remains the primary surface. `분석` opens the existing media-draft analysis state over the chat flow; `닫기` returns to standalone chat without deleting the draft.
- Text-only questions are sent from the standalone composer. The media-draft state has one primary `분석 시작` action rather than a second text-only send action.
- Photo and video selection share one `사진·영상 첨부` action that opens the operating-system media picker. Do not build a separate live browser camera preview. The media draft accepts multiple files for one analysis, keeps every selected item individually visible and removable, and starts analysis only after review.
- The standalone chat shows the currently applied shot, club, and goal context and provides a direct edit action so personal swing questions do not require repeated follow-up questions.
- Photos are accepted only as limited single-frame evidence. The prototype must not present a still image as a complete motion-sequence analysis.
- Non-golf media has an explicit rejected state and never receives a golf correction result.
- User questions and FEEL are response context; the UI states that attached media is observed independently before the answer compares it with the question.
